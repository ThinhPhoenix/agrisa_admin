...
export function useTableData(data = [], config = {}) {
  ...
  const handleFormSubmit = useCallback((values) => {
    const { search, searchButton, clearButton, spacer, ...filterValues } = values;

    if (search !== undefined) {
      setSearchText(search || "");
    }

    // Only update filters that are provided in the form (exclude button fields)
    const newFilters = {};
    Object.entries(filterValues).forEach(([key, value]) => {
      // Skip button and custom fields
      if (!key.endsWith('Button') && key !== 'spacer') {
        newFilters[key] = value !== undefined ? value : '';
      }
    });
