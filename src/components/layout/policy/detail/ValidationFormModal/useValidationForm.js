...
export default function useValidationForm({
  ...
  // Helper to apply AI data with a smooth loading state to avoid UI jank
  const applyAIData = async (next) => {
    try {
      setApplyingAI(true);

      // If enabling AI data and we have latestValidation, apply values synchronously
      if (next && latestValidation && form) {
        const mismatchesArray = latestValidation.mismatches
          ? Object.entries(latestValidation.mismatches).map(([key, value]) => ({
              field: key,
              expected:
      ...
      // Toggle the flag that other UI uses for disabling inputs
      setUseAIData(next);

      // Wait for next paint to avoid UI jank
